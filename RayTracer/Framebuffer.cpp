...
void Framebuffer::DrawPoint(int x, int y, const color_t& color)
{
    ...
    destColor.r = ((color.r * alpha) + (destColor.r * invAlpha)) >> 8; // >> 8 == / 255
...
void Framebuffer::DrawTriangle(int x1, int y1, int x2, int y2, int x3, int y3, const color_t& color)
{
    ...
    // draw line from x2,y2 -> x3,y3
    DrawLine(x2, y2, x3, y3, color);

    // draw line from x3,y3 -> x1,y1
...
void Framebuffer::DrawImage(int x1, int y1, Image* image)
{
    for (int y = 0; y < image->colorBuffer.height; y++)
    {
        ...
        for (int x = 0; x < image->colorBuffer.width; x++)
        {
            ...
            color_t color = ((color_t*)colorBuffer.data)[sx + (sy * colorBuffer.width)] = ((color_t*)image->colorBuffer.data)[x + (y * image->colorBuffer.width)];
            DrawPoint(sx, sy, color);
        }
    }
}
