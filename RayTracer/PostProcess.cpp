...
namespace nPostProcess
{
	...
	void Threshold(const ColorBuffer& colorBuffer, uint8_t threshold)
	{
		for (int i = 0; i < (colorBuffer.width * colorBuffer.height); i++)
		{
			...
			int16_t r = (color.r >= threshold) ? color.r : 0;
			int16_t g = (color.g >= threshold) ? color.g : 0;
			int16_t b = (color.b >= threshold) ? color.b : 0;
